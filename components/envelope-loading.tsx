...
export default function EnvelopeLoading({ language, onComplete }: EnvelopeLoadingProps) {
  ...
  useEffect(() => {
    if (typingStarted) {
      ...
      } else if (titleIndex === fullTitle.length && showStamp && !paperExpanded) {
        const expandTimeout = setTimeout(() => {
          setPaperExpanded(true)
        }, 600) // delay before expansion
        return () => clearTimeout(expandTimeout)
      }
    }
  }, [typingStarted, titleIndex, fullTitle, showStamp, paperExpanded])
  ...
  useEffect(() => {
    if (paperExpanded) {
      const completeTimeout = setTimeout(() => {
        setStage("completed")
        onComplete()
      }, 1000) // delay to allow paper expansion animation
  ...
  return (
    <>
      {/* Expanding paper layer */}
      {paperExpanded && (
        <div className=" paper-style fixed inset-0 z-50 bg-[#f9f6f1] flex items-center justify-center flex-col transition-all duration-1000 animate-expandPaper">
          <p className="text-2xl sm:text-4xl text-[#795548] font-semibold font-serif">{fullTitle}</p>
          <p className={`text-xl sm:text-2xl text-[#795548] font-semibold font-serif pt-5 mt-5`}>
      ...
      <div className="h-screen w-full flex items-center justify-center bg-[#f9f6f1] relative overflow-hidden">
        ...
        >
          ...
          <div
            className={`absolute flex flex-col items-center py-5 justify-start paper-style w-full h-full transition-all duration-700 ${
              isEnvelopeOpen ? '-translate-y-16 delay-800 animate-paperRise' : ''
            } ${paperExpanded ? 'opacity-0' : ''}`}
          >
            <p className="text-xl sm:text-2xl text-[#795548] font-semibold font-serif">
              {titleText}
            </p>
            ...
            <p className={`text-xl sm:text-2xl text-[#795548] font-semibold font-serif pt-5 mt-5 transition-all duration-700 ${showStamp ? "scale-100 opacity-100" : "scale-0 opacity-0"}`}>
              {language === "en" ? "M & Y" : "م و ي"}
            </p>
          </div>
                ...
                <svg width="72" height="100" viewBox="0 0 69 100" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <g>
                    ...
                    <polyline id="arrow" points="-6,-9 12,0 -6,9 -1,0" transform="rotate(-15)" fill="black">
                      <animateMotion rotate="auto" begin="1s" dur="1s" repeatCount="1" fill="freeze">
